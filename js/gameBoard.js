...
export function addShip(ship) {
  ...
  let randomBool = Math.random() < .5
  let isHorizontal = randomBool
  let randomStartIdx = Math.floor(Math.random() * (width * width))
  
  // check if ship spawn point is okay
  let validStart = isHorizontal ? randomStartIdx <= width * width - ship.length ? randomStartIdx : width * width - ship.length : randomStartIdx <= width * width - width * ship.length ? randomStartIdx : randomStartIdx - ship.length * width + width
  ...
  let valid;
  if(isHorizontal) {
    valid = shipBlocks.every((block, i) => shipBlocks[0].id % width !== width - (shipBlocks.length - (i + 1)))
  } else {
    valid = shipBlocks.every((block, i) => shipBlocks[0].id < 90 + (width * i + 1))
  }
